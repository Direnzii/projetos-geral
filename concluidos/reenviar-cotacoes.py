...
class reenviar:
    ...
    def cotacoes(self):
        ...
        if self.verificar_se_retornou_algo() == True: #True quer dizer que retornou sim algo na primeira busca
    ...
    def contar_enviar(self):
        botoes_enviar = pagina.locator('xpath=/html/body/div[3]/form[1]/div[3]/div/div[3]/table/tbody/tr').count() # contar a quantidade de botões enviar tem na pagina
        time.sleep(0.5)
        if botoes_enviar == 1:
            primeiro_enviar = pagina.locator('xpath=/html/body/div[3]/form[1]/div[3]/div/div[3]/table/tbody/tr/td[6]/div/button/span').get_attribute('class')
        contador = 0
    ...
    def reenviar_cotacoes(self):
        ...
        botao_avancar = pagina.locator(
            'xpath=/html/body/div[3]/form[1]/div[3]/div/div[3]/table/tfoot/tr/td/span[4]').get_attribute('class') #ver se da pra clicar no botão de passar a pagina
...
def erro_para_rodar_o_programa():
        print('Você fez merda, para rodar o programa são necessarios 2 argumentos, Data que quer reenviar e CNPJ do fornecedor.')
